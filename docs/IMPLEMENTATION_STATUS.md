...
- [ ] Funded V3 browser-wallet completion, Cast/runtime example coverage, public-registry consumer smoke, and public npm releases with provenance. The current token reaches the authorized publish boundary, but npm rejects provenance because the source repository is private; no package was published.
...
- [ ] Full live-V3 final-origin metadata/discovery cutover, funded E2E matrix and monitored release. The address-free draft web/agent release and v2 metadata cutover are complete, but they are not live-V3 evidence.

Every unchecked item is pending. See `docs/V3_ACCEPTANCE_MATRIX.md`: only rows carrying explicit evidence pass. V3 deployment, funded V3 wallet/browser execution, public npm, paid-x402 runtime, audit, soak and operations rows remain `NOT RUN` or explicitly blocked.

## Current phase

The following is the **historical/current v2 state plus a hosted draft integration release**, not v3 completion: the v2 Base Sepolia contract is deployed, source-verified, and exercised with a real multi-account write smoke. Its metadata base URI was cut over on-chain from localhost to `https://sepbase.vercel.app/api/metadata/` in transaction `0x80a501b33c00e93e2bb6b87f0623b53ed3a42d054cdaa64d37097d3d11032c4b`, then verified after five confirmations; evidence is versioned under `evidence/v2-metadata-cutover/`. Production deployment `dpl_5UDQ59oGJX6An7NKnnEYf3dHBgEj` now serves the current web/agent source at `https://sepbase.vercel.app`: schema-4 discovery separates current-v2 `/api/mcp` from opt-in `/api/v3/mcp`, free x402 quotes are HMAC-authenticated, and paid execution is explicitly `activation-gated`. The address-free V3 manifest remains draft and fail-closed. Hosted smoke/browser/log evidence is versioned under `evidence/hosted-release/`; remaining value-bearing V3 gates are still pending.

Separately, the repository contains the seven V3 Solidity implementations, generated per-module ABIs, an address-free `releaseStatus: "draft"` manifest, V3 SDK/API/MCP adapters and local tests. `pnpm artifacts:v3:check` validates seven modules and rejects ten drift/partial-release cases. Hosted draft parity does not supply V3 addresses, deployment receipts, Base Sepolia source verification, public package publication or paid execution.
...
2. Use an authenticated production RPC and add availability/rate monitoring for hosted read APIs.
...
4. Add a WalletConnect project ID and complete funded desktop/mobile V3 wallet E2E, including a live non-empty marketplace fixture.
5. Publish `@sepbase/sdk`, `@sepbase/react` and `@sepbase/mcp`, configure per-package GitHub OIDC trusted publishing, revoke the bootstrap token and repeat consumer smoke from the public registry. Workflow run `29246024626` passed the entire validation matrix and generated signed SDK provenance, but npm rejected it with `E422` because provenance does not support a private GitHub source repository. No package exists yet. The reviewed source must move to a public repository before retrying with provenance; publishing without provenance is not an accepted fallback.
...
The Vercel production environment has the final HTTPS site origin, current agent artifacts and a production quote authenticator; v2 on-chain metadata points to that origin. `pnpm release:check` remains intentionally blocked until the server-only authenticated `RPC_URL`, WalletConnect project ID and a deployed/live V3 manifest with matching on-chain metadata exist. Local development continues to use `http://localhost:3000`.
