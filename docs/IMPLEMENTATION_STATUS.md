...
- [x] Encrypted CAS source is now implemented at the internal paid boundary with AES-256-GCM AAD-bound envelopes, PostgreSQL serializable transactions, authorization/plan/quote uniqueness, fencing leases, a checksum-guarded schema migration and client-protocol tests. The Neon resource, production CAS secrets and schema are provisioned; fresh hosted authenticated smoke, backup/restore and distributed concurrency evidence remain pending.
...
The following is the **historical/current v2 state plus a hosted draft integration release**, not v3 completion: the v2 Base Sepolia contract is deployed, source-verified, and exercised with a real multi-account write smoke. Its metadata base URI was cut over on-chain from localhost to `https://sepbase.vercel.app/api/metadata/` in transaction `0x80a501b33c00e93e2bb6b87f0623b53ed3a42d054cdaa64d37097d3d11032c4b`, then verified after five confirmations; evidence is versioned under `evidence/v2-metadata-cutover/`. Production deployment `dpl_EbnrRgAhFFZ66J7UafLnf6t38oFy` serves the current web/agent/CAS source at `https://sepbase.vercel.app`: schema-4 discovery separates current-v2 `/api/mcp` from opt-in `/api/v3/mcp`, free x402 quotes are HMAC-authenticated, and paid execution is explicitly `activation-gated`. The address-free V3 manifest remains draft. Neon/CAS provisioning occurred after this release and requires redeployment; the paid route remains fail closed. Hosted smoke/browser/log evidence is versioned under `evidence/hosted-release/`; remaining value-bearing V3 gates are still pending.
