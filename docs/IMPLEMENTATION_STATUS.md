...
- [ ] Funded V3 browser-wallet completion, Cast/runtime example coverage, public-registry consumer smoke, and public npm releases with provenance. The first package workflow reached npm and emitted signed provenance, but registry authorization rejected package creation; no package was published.
...
5. Publish `@sepbase/sdk`, `@sepbase/react` and `@sepbase/mcp`, configure per-package GitHub OIDC trusted publishing, revoke the bootstrap token and repeat consumer smoke from the public registry. Workflow run `29244090135` passed the entire validation matrix and generated signed SDK provenance, but npm rejected first package creation with `E404` because the stored publisher credential is invalid or lacks scope write permission. No package exists yet; first publication requires a fresh read-write granular token with bypass 2FA, after which OIDC should replace it.
