...
The following is the **historical/current v2 product plus a deployed V3 candidate**, not v3 completion: the v2 Base Sepolia contract is deployed, source-verified, and exercised with a real multi-account write smoke. Its metadata base URI was cut over on-chain to `https://sepbase.vercel.app/api/metadata/` and verified after five confirmations. The prior production web deployment still represents the hosted draft baseline until the candidate source is redeployed and final-origin smoked. Neon/CAS is provisioned, while paid execution remains fail closed. Hosted evidence is versioned under `evidence/hosted-release/`; V3 chain evidence is under `evidence/v3-deployment/`.
