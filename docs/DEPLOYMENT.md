...
At the 2026-07-13 audit, current source is `Ready` on protected Preview deployment `dpl_2pEWFeWsqSestVf7n4xkZwLicAci` at `https://sepbase-fd1ancpvz-yusufky63s-projects.vercel.app`. Vercel-authenticated checks pass for OpenAPI `1.7.0`, schema-4 agent discovery, `llms.txt`, `/me`, the migration-aware 39-tool V3 MCP inventory through an origin-less server-client request, V3 draft status and the expected fail-closed V3/x402 routes, with zero recent deployment error logs. A forced Preview `Origin` correctly returns `403` because the route is locked to the configured canonical origin. Preview currently has no `NEXT_PUBLIC_SITE_URL`, server-only quote authentication or authenticated `RPC_URL`, so generated canonical/OpenAPI URLs fall back to `http://localhost:3000` and the free quote remains `503 QUOTE_AUTHENTICATION_NOT_CONFIGURED`. Anonymous `pnpm hosted:check` also receives the intended Deployment Protection `302`. The existing `https://sepbase.vercel.app` production alias remains stale; do not promote until the release gates above pass.
