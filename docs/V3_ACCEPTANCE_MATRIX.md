...
Current evidence snapshot (2026-07-13): `pnpm contracts:test` passed 91/91 tests, of which 62 are V3 unit/edge/fuzz/invariant/acceptance tests and 29 are historical v2 tests; `pnpm artifacts:v3:check` passed one positive and ten fail-closed artifact cases for seven modules; normalization validation passed 8 accepted/6 rejected fixtures; SDK 55/55, MCP 34/34, React 4/4 and web 229/229 tests passed. `pnpm packages:smoke` also packs all three packages and installs them into an isolated NodeNext consumer, but this is not a public-registry/provenance result. These are source evidence, not funded wallet or external-issuer E2E. `pnpm examples:v3:typecheck` compiles the workspace SDK/Viem/Wagmi/React/MCP consumer fixture, but does not cover Cast, runtime behavior, a public-npm install or deployed V3. `V3_MIGRATION_SOURCE_BLOCK=44054186 pnpm migration:v3:dry-run` reproducibly generated `evidence/v3-migration/84532-44054186.json` (6/6 active names eligible; embedded `reportSha256` `492ab72338895dc101b3ce4b0b4bb1971d7521fd9797c28ddb20882ded5b4551`), but this is preflight only and does not make a migration E2E row pass. Protected Preview `dpl_2pEWFeWsqSestVf7n4xkZwLicAci` includes the current migration UI/SDK/MCP additions and exposes origin-less V3 MCP `tools/list` with 39 tools, including account-scoped migration eligibility. It still cannot satisfy live final-origin rows: all seven V3 addresses are null, paid execution is unavailable, and Preview canonical/OpenAPI origin falls back to localhost because Preview env is unprovisioned. These results prove only the specifically cited rows below. No V3 Base Sepolia deployment, migration claim transaction, complete live final-origin V3 smoke, public npm install/provenance, independent audit, soak, external attestation issuer or funded paid execution was run.
...
| I04 | MCP initialize/tools | Version, Origin/body/rate rules and no signer path | PARTIAL: local MCP HTTP tests pass Origin/body/stateless/no-signer rules; protected Preview `dpl_2pEWFeWsqSestVf7n4xkZwLicAci` returns 39 V3 tools to an origin-less server client and rejects the non-canonical Preview Origin. Hosted rate/abuse policy and final canonical-origin smoke remain NOT RUN |
