...
Status: **selected local/source, npm-publication and Base Sepolia deployment rows now have reproducible evidence; other release rows remain pending**. Passing deployment checks does not satisfy live-V3 hosted-origin, funded browser, audit, soak or paid-x402 runtime rows.

Evidence columns use `NOT RUN`, `PASS <command/artifact>`, or `BLOCKED <reason>`. A checkbox is not evidence.

Current evidence snapshot (2026-07-13): `pnpm contracts:test` passed 91/91 tests (62 V3 and 29 historical v2); `pnpm artifacts:v3:check` passed one positive and ten fail-closed artifact cases; SDK 55/55, MCP 34/34, React 4/4 and web 238/238 tests passed. Public `@sepbase/sdk`, `@sepbase/react` and `@sepbase/mcp` `0.1.0` packages have SLSA provenance and an anonymous clean-consumer smoke (`evidence/npm-release/2026-07-13-v0.1.0.json`). The seven-contract Base Sepolia suite was deployed from source commit `a2d3ac739a3ae5ccaf63fd4bc20c97b124de7c72`; eight canonical receipts succeeded in blocks `44094615`-`44094623`, all seven sources are verified, and pinned-block manifest promotion produced candidate `sha256:afd20a1a0ac6608a1ea1528c111600f4d74f5faf33b15b915cb8137598dada34` (`evidence/v3-deployment/2026-07-13.json`). Clean production deployment `dpl_D9U98US6hoi1tyx9umf3woRwUHmi` passed candidate final-origin parity and paid fail-closed smoke (`evidence/hosted-release/2026-07-13-v3-candidate-production.json`). No migration claim, funded V3 wallet flow, authenticated production CAS mutation, audit, soak, managed attestation issuer or funded paid execution has run; paid execution remains unavailable.
...
| ID | Scenario | Required result | Evidence |
|---|---|---|---|
| I01 | npm install in clean fixture | Exact public versions install with provenance | PASS workflow `29246721839` + anonymous `pnpm exec tsx scripts/package-release-smoke.ts --registry 0.1.0` + `evidence/npm-release/2026-07-13-v0.1.0.json` |
| I02 | TypeScript/Viem/Wagmi/Cast snippets | Compile/run against fixture; no undefined placeholders | NOT RUN |
| I03 | Seven-address manifest/ABI/version/profile/fixture/attestor or helper-binding mismatch | Typed fail-closed error before reads/writes are trusted | PASS candidate promotion + `pnpm artifacts:v3:check` (1 positive + 10 fail-closed drift cases) + `pnpm --filter @sepbase/sdk test` (`v3-manifest.test.ts`) + `evidence/v3-deployment/2026-07-13.json` |
| I04 | MCP initialize/tools | Version, Origin/body/rate rules and no signer path | PARTIAL: local MCP HTTP tests pass Origin/body/stateless/no-signer rules; Production candidate `dpl_D9U98US6hoi1tyx9umf3woRwUHmi` passes canonical-origin smoke with 8 v2 and 39 V3 tools. Hosted rate/abuse load evidence remains NOT RUN |
...
| J03 | Duplicate/concurrent payment identifier | One order/execution; same status response | SOURCE PASS authorization hash + `cas/store.test.ts` global uniqueness/lease fencing + external-client wire test; provisioned distributed-service concurrency E2E NOT RUN |
...
| J05 | Crash after commit/submission/confirmation | Resume/reconcile; no blind replay | SOURCE PASS AES-256-GCM reservation reload + Workflow continuation + CAS response-loss/fencing tests; managed-Postgres recovery drill NOT RUN |
...
| J09 | Secret/log scan | No key, auth, signed payload or private RPC leakage | SOURCE PASS `cas/crypto.test.ts` AAD/tamper/random-IV coverage, authenticated internal route and public-response omission tests; hosted log/release secret scan NOT RUN |

## K. Hosted release, security and operations

| ID | Scenario | Required result | Evidence |
|---|---|---|---|
| K01 | Final-origin discovery | Metadata, seven ABI/address/version checksums, four registry bindings, Universal Resolver/MarketLens bindings, normalization profile/attestor, manifests, OpenAPI, docs and llms parity | PARTIAL: candidate final-origin 4-page/8-ABI/manifest/OpenAPI/llms/MCP/V3-market parity PASS; live promotion and explorer/wallet metadata fetch remain NOT RUN |
...
| K09 | One-time suite configuration | Immutable configurator wires controller/public resolver/migration/marketplace exactly once; owner is separate; repeat/invalid wiring rejects | PASS canonical `configureSuite` receipt `0xf9b02c9db702455b296443f35e0d13ec70b699e267e89a108da5d6d8614e5f7d`, pinned-block SDK binding verification and local repeat/invalid rejection tests; `evidence/v3-deployment/2026-07-13.json` |
...
Additional V3 browser, Base Sepolia, live-V3 hosted, audit, soak and paid-execution suites are still required. Any row that remains `NOT RUN` is pending even if an adjacent source test passes.
