...
Status: **selected local/source and npm-publication rows now have reproducible evidence; other release rows remain pending**. Passing v2 tests does not satisfy a v3 row, and local Foundry/SDK/API evidence does not satisfy Base Sepolia, live-V3 hosted-origin, audit, soak or paid-x402 runtime rows.

Evidence columns use `NOT RUN`, `PASS <command/artifact>`, or `BLOCKED <reason>`. A checkbox is not evidence.

Current evidence snapshot (2026-07-13): `pnpm contracts:test` passed 91/91 tests, of which 62 are V3 unit/edge/fuzz/invariant/acceptance tests and 29 are historical v2 tests; `pnpm artifacts:v3:check` passed one positive and ten fail-closed artifact cases for seven modules; normalization validation passed 8 accepted/6 rejected fixtures; SDK 55/55, MCP 34/34, React 4/4 and web 238/238 tests passed. `pnpm packages:smoke` validates locally packed artifacts. Public workflow `29246721839` then published `@sepbase/sdk`, `@sepbase/react` and `@sepbase/mcp` at exact version `0.1.0` with SLSA provenance; `pnpm exec tsx scripts/package-release-smoke.ts --registry 0.1.0` installed them anonymously into an isolated strict-NodeNext consumer and passed type/runtime checks. Integrity and attestation evidence is `evidence/npm-release/2026-07-13-v0.1.0.json`. `pnpm examples:v3:typecheck` compiles the workspace SDK/Viem/Wagmi/React/MCP consumer fixture, but does not cover Cast or deployed-V3 runtime behavior. `V3_MIGRATION_SOURCE_BLOCK=44054186 pnpm migration:v3:dry-run` reproducibly generated `evidence/v3-migration/84532-44054186.json` (6/6 active names eligible; embedded `reportSha256` `492ab72338895dc101b3ce4b0b4bb1971d7521fd9797c28ddb20882ded5b4551`), but this is preflight only. `V3_BROADCAST=false pnpm contracts:deploy:v3` passed all source, artifact, Safe, configurator, chain and Circle test USDC checks without submitting a transaction; production Workflow workflow/step health and the official facilitator's x402 V2 `exact` support on `eip155:84532` were also observed. Evidence is `evidence/v3-preflight/2026-07-13.json`; these are prerequisite checks, not a deployment or paid runtime. Production draft `dpl_EbnrRgAhFFZ66J7UafLnf6t38oFy` passes canonical-origin smoke for four pages, eight ABI artifacts and exact 8+39 MCP inventories with authenticated RPC, WalletConnect connector rendering and fail-closed encrypted-CAS/paid routes; evidence is `evidence/hosted-release/2026-07-13-cas-source-production.json`. The deployment came from a dirty working tree. All seven V3 addresses remain null and paid execution remains unavailable, so neither packages nor hosted source satisfy live-V3 final-origin rows. No V3 deployment, migration claim, funded V3 wallet flow, audit, soak, external attestation issuer or funded paid execution was run.
...
| I04 | MCP initialize/tools | Version, Origin/body/rate rules and no signer path | PARTIAL: local MCP HTTP tests pass Origin/body/stateless/no-signer rules; Production draft `dpl_EbnrRgAhFFZ66J7UafLnf6t38oFy` passes canonical-origin smoke with 8 v2 and 39 V3 tools. Hosted rate/abuse load evidence remains NOT RUN |
