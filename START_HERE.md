...
Bugünkü yerel kanıt Faz V3-1/V3-2 kaynaklarının önemli bölümünü ve V3 SDK/read-API/opt-in MCP adaptörlerini kapsar: `pnpm contracts:test` 91/91 (62 V3 + 29 historical v2), SDK 55/55, MCP 34/34, React 4/4 ve web 229/229 geçer; V3 artifact validator yedi modülü, bir positive ve on fail-closed drift senaryosuyla doğrular. `V3_MIGRATION_SOURCE_BLOCK=44054186 pnpm migration:v3:dry-run` komutu 6/6 aktif v2 adı içeren sabit-blok preflight raporunu yeniden üretir; bu bir V3 deployment/claim/cutover kanıtı değildir. Bu sonuçlar fazları bütünüyle kapatmaz; V3 Base Sepolia deployment, funded browser-wallet/paid-x402 E2E, public npm provenance, bağımsız audit ve soak hâlâ yoktur. Current-v2 MCP `/api/mcp`, 39 araçlık opt-in V3 MCP ise `/api/v3/mcp` yolundadır.
